...
############################End of the setup############################################   
	...
	plt.close()
#######################
'''	data = np.genfromtxt("free-energy-landscape-rmsd-60-BE_kcal.dat")
	print (data.shape)
	print(min(data[:,3]))
	print(max(data[:,3]))
	print (data.shape)
	x = data[:,0].reshape(60,-1)
	x = gaussian_filter(x, sigma=.7)
	#x = scipy.ndimage.zoom(x, 2)
	y = data[:,1].reshape(60,-1)
	y = gaussian_filter(y, sigma=.7)
	#y = scipy.ndimage.zoom(y, 2)
	z = data[:,3].reshape(60,-1)
	z = gaussian_filter(z, sigma=.7)
	#z = scipy.ndimage.zoom(z, 2)
	z = np.ma.masked_where(z >= 2.7, z)

	#plt.contour(x,y,z, colors='k')
	plt.contourf(x,y,z,9, vmin = 0., vmax = 3, cmap='jet')
	m = plt.cm.ScalarMappable(cmap='jet')
	plt.grid(which='major',linestyle = '--', linewidth=1, color='k', alpha = 0.3)
	#plt.grid(which='minor',linestyle = '--', linewidth=1, color='k', alpha = 0.15)
	plt.xlabel(r'RMSD ($\AA$)')
	plt.ylabel('PL1 Diagonal Distance ($\AA$)')
	plt.xlim(3, 5.5)
	plt.ylim(26, 34)
	plt.clim(0.0,3)
	plt.colorbar(m, boundaries=np.linspace(0, 3, 9))
	save("6P07-APO-BE")
	plt.close()
#######################
	data = np.genfromtxt("free-energy-landscape-rmsd-60-CF_kcal.dat")
	print (data.shape)
	print(min(data[:,3]))
	print(max(data[:,3]))
	print (data.shape)
...
#######################
	...
	data = np.genfromtxt("free-energy-landscape-rmsd-60-All_kcal.dat")
	print (data.shape)
	print(min(data[:,3]))
	print(max(data[:,3]))
	print (data.shape)
